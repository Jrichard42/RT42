...
/*   Updated: 2017/03/19 11:55:18 by jrichard         ###   ########.fr       */
...
int			create_camera(t_kvlexer *token, t_rt *rt)
{
	if (!(rt->camera = malloc(sizeof(t_camera))))
		return(-1);
	get_as_int(token, "FOV", &(rt->camera->fov));;
	rt->camera->up = create_vector3f(0.0, -1.0, 0.0);
	get_as_vector3f(token, "POS", &(rt->camera->pos));
	get_as_vector3f(token, "LOOKAT", &(rt->camera->lookatpoint));
