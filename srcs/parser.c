...
/*   Updated: 2017/03/30 15:20:44 by jrichard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static t_ptr_type	ptr_type[15] = {{"SPHERE\0", &create_sphere},
			...
			{"PARA\0", &create_paraboloide}, {"INCLUDE\0", &parser_text_mat}};
	int					i;
	...
	while (i < 15)
	{
		...
		{
			if (!ptr_type[i].create(token, rt) && i != 14)
