...
float 				*do_cl_vector_calc3f(t_cl *cl, char *ker_name, float v1[3],
										  float v2[3])
{
	cl_mem 			memobj[3];
	float 			*res;
	...
	clEnqueueReadBuffer(cl->queue, memobj[2], CL_TRUE, 0, 3 * sizeof(float), (void *)&res, 0, NULL, NULL);
...
t_vector3f			dot_vector3f_cl(t_cl *cl, t_vector3f v1, t_vector3f v2)
{
	float 			*va;
	float 			*vb;
	float 			*vc;
